...
**한 지시당 실행 시간.** 사람이 프롬프트를 친 시각부터, 다음 사람 프롬프트 전 **마지막 메인 어시스턴트 메시지**까지의 초.
트랜스크립트 구간(약 30일) 기준이며, `human` 으로 분류된 턴만 시작점으로 잡는다.

- 자율 모드(ulw · ralph · team)가 Stop hook 으로 이어서 돈 시간은 **포함**된다 — 그게 "한 번 시켜서 걸린 시간"이다
- 결과를 검토하며 다음 지시를 고민한 시간은 **빠진다** (다음 사람 턴이 아니라 마지막 어시스턴트 메시지까지만 잰다)
- 서브에이전트(사이드체인) 메시지는 시작·종료 판정에 쓰지 않는다 (메인 루프가 그 결과를 받아 이어 가므로 이미 포함된다)
- 사람 턴 뒤에 어시스턴트 메시지가 하나도 없으면(연속 입력·중단) 그 지시는 세지 않는다

| 필드 | 뜻 |
|---|---|
| `n` | 측정한 지시 수 |
| `median_sec` · `p25_sec` · `p75_sec` · `p90_sec` · `max_sec` · `mean_sec` | 분위 · 최장 · 평균 (초) |
| `total_sec` | 지시들이 돌아간 시간 합계 — "AI 가 나 대신 일한 시간"의 하한 |
| `buckets` | `1분 이하` · `1~5분` · `5~15분` · `15~60분` · `1시간 초과` 건수 |
| `longest` | 가장 오래 돈 지시 5개 (세션 · 프로젝트 · 시작 시각). **요약 JSON 에는 옮기지 않는다** |

`speech.inter_prompt_gap_sec`(프롬프트 사이 간격) 과 다르다 — 그쪽은 사람이 다음 지시를 내기까지의 간격이라 검토 시간이 섞여 있다.
밤새 돌린 team 세션처럼 한 지시가 수 시간을 넘는 경우가 있어 **평균보다 중앙값**으로 보고한다.
...
| 2026-08-28 | `turn_durations` 블록 추가 — 한 지시당 실행 시간 (저장소 관리자) | 같은 날 첫 회차 보고서에 반영. 이전에 뽑은 JSON 은 `workflow.turn_duration` 이 없어 경고만 뜬다 |
